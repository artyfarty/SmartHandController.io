...
void Encoders::init(UI *ui_instance) {
  ...
  void Encoders::poll() {
      ...
      /*
      if (prevDiff1 > 0 && prevDiff2 > 0 && prevDiff3 > 0) {
          should_slew = true;
          slew_dir = 1;
          VF("MSG: EncSlew+ "); V(prevDiff1); V(","); V(prevDiff2); V(","); VL(prevDiff3);
      } else if (prevDiff1 < 0 && prevDiff2 < 0 && prevDiff3 < 0) {
          should_slew = true;
          slew_dir = -1;
          VF("MSG: EncSlew- "); V(prevDiff1); V(","); V(prevDiff2); V(","); VL(prevDiff3);
      } else {
      ...
      if (should_slew) {
          ...
          VF("MSG: EncSlew spd "); VL(encoderSpeed);
          float guideSpeed = encoderSpeed / 600 * 360 / 10;

          ui.setCustom GuideRate(guideSpeed);
